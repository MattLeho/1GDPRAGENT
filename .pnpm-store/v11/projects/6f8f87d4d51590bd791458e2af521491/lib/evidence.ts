...
export const locatorSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('json_record'), record: z.number().int().nonnegative(), pointer: z.string().regex(/^(|\/.*)$/).default('') }).strict(),
  ...
  z.object({ type: z.literal('text_line'), line: z.number().int().positive(), column_start: z.number().int().nonnegative().optional(), column_end: z.number().int().positive().optional() }).strict(),
  z.object({ type: z.literal('text_byte_span'), byte_start: z.number().int().nonnegative(), byte_end: z.number().int().positive() }).strict().refine(v => v.byte_end > v.byte_start),
  z.object({ type: z.literal('xml_element'), xpath: z.string().min(1), attribute: z.string().min(1).optional() }).strict(),
  ...
  z.object({ type: z.literal('pdf_page_block'), page: z.number().int().positive(), block: z.number().int().nonnegative() }).strict(),
  z.object({ type: z.literal('pdf_region'), page: z.number().int().positive(), x: z.number().nonnegative(), y: z.number().nonnegative(), width: z.number().positive(), height: z.number().positive() }).strict(),
  z.object({ type: z.literal('office_paragraph'), paragraph: z.number().int().nonnegative(), run: z.number().int().nonnegative().optional() }).strict(),
  z.object({ type: z.literal('office_table_cell'), table: z.number().int().nonnegative(), row: z.number().int().nonnegative(), column: z.number().int().nonnegative() }).strict(),
  z.object({ type: z.literal('spreadsheet_cell'), sheet: z.string().min(1), row: z.number().int().positive(), column: z.number().int().positive(), address: z.string().optional() }).strict(),
  z.object({ type: z.literal('slide_shape'), slide: z.number().int().positive(), shape: z.number().int().nonnegative() }).strict(),
  z.object({ type: z.literal('slide_notes'), slide: z.number().int().positive(), note: z.number().int().nonnegative().default(0) }).strict(),
  z.object({ type: z.literal('email_header'), message: z.number().int().nonnegative(), header: z.string().min(1), occurrence: z.number().int().nonnegative().default(0) }).strict(),
  z.object({ type: z.literal('email_mime_part'), message: z.number().int().nonnegative(), part: z.string().min(1) }).strict(),
  z.object({ type: z.literal('email_attachment'), message: z.number().int().nonnegative(), part: z.string().min(1), filename: z.string().optional() }).strict(),
  z.object({ type: z.literal('calendar_component'), component: z.string().min(1), uid: z.string().optional(), occurrence: z.number().int().nonnegative().default(0), property: z.string().optional() }).strict(),
  z.object({ type: z.literal('vcard_property'), card: z.number().int().nonnegative(), property: z.string().min(1), occurrence: z.number().int().nonnegative().default(0) }).strict(),
  ...
  z.object({ type: z.literal('video_frame'), timestamp_ms: z.number().int().nonnegative(), frame: z.number().int().nonnegative().optional() }).strict(),
  z.object({ type: z.literal('subtitle_cue'), cue: z.number().int().positive(), start_ms: z.number().int().nonnegative(), end_ms: z.number().int().positive() }).strict(),
  z.object({ type: z.literal('geospatial_feature'), feature: z.union([z.string(),z.number().int()]), coordinate: z.number().int().nonnegative().optional(), segment: z.number().int().nonnegative().optional() }).strict(),
  z.object({ type: z.literal('database_table_row'), table: z.string().min(1), row_key: z.record(z.string(),z.unknown()) }).strict(),
  z.object({ type: z.literal('database_cell'), table: z.string().min(1), row_key: z.record(z.string(),z.unknown()), column: z.string().min(1) }).strict(),
  z.object({ type: z.literal('archive_member'), member_path: z.string().min(1), outer_artifact_id: z.string().uuid().optional(), nested_member_chain: z.array(z.string()).default([]), member_ordinal: z.number().int().nonnegative().optional() }).strict(),
])
