...
async def manual_node(body: ManualNodeRequest):
    try:
        ledger=EvidenceLedger(); run_id=await ledger.create_analysis_run("manual_graph_edit","task1-manual-v1",configuration={"ui":"graph"})
        ...
        assertion_id=await ledger.create_assertion(assertion); projected=await GraphProjectionService().project_assertion(assertion_id)
...
async def project(body: ProjectRequest):
    try: return await GraphProjectionService().project_assertion(body.assertion_id)
    except ValueError as exc: raise HTTPException(status_code=409,detail=str(exc)) from exc


async def _human_mutation_assertion(subject_ref: str, predicate: str, object_ref: str) -> UUID:
    ledger=EvidenceLedger(); run_id=await ledger.create_analysis_run("manual_graph_edit","task1-manual-v1",configuration={"ui":"graph"})
...
async def manual_retire(body: ManualRetireRequest):
    try:
        assertion_id=await _human_mutation_assertion(body.subject_ref,"SUPERSEDES",f"Claim:retire:{body.node_id}")
        await GraphProjectionService().retire_node(assertion_id,body.node_id)
        return {"success":True,"assertion_id":assertion_id,"node_id":body.node_id}
    except ValueError as exc: raise HTTPException(status_code=409,detail=str(exc)) from exc


@router.post("/manual-merge")
async def manual_merge(body: ManualMergeRequest):
    try:
        assertion_id=await _human_mutation_assertion(body.subject_ref,"RELATES_TO",f"Claim:merge:{body.source_node_id}:{body.target_node_id}")
        await GraphProjectionService().merge_nodes(assertion_id,body.source_node_id,body.target_node_id)
...
async def backfill_graph():
    return {"updated":await GraphProjectionService().backfill_legacy_node_ids()}
...
async def onsit_bulk(body: OnsitBulkRequest):
    try:
        assertion_id=await _human_mutation_assertion("local-subject","HUMAN_ASSERTED",f"Claim:onsit-bulk:{body.action}:{','.join(map(str,body.finding_ids))}")
