...
import { Trash2, ArrowRight, Eye, Calendar } from "lucide-react"
...
export function ScopeSelector() {
    ...
    const handleSubmit = async () => {
        if (!targetUrl || !selectedIdentity) {
            toast.error('Complete the company and identity steps before creating the request')
            return
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                toast.error('Request was not sent', { description: result.message })
                if ('partial' in result && result.partial && 'requestId' in result) {
                    reset()
                    router.push('/dashboard/requests')
                }
            }
        } catch {
            toast.error('Request could not be created', { description: 'The server did not complete the operation. Nothing is shown as sent.' })
        } finally {
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Create the request record. Delivery occurs only when a configured workflow succeeds; otherwise it remains queued or drafted for review.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-zinc-800 dark:bg-zinc-900 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="sm:text-right">
                            ...
                            <p className="font-semibold text-slate-900 dark:text-zinc-100">{selectedIdentity?.contactName || "Identity not selected"}</p>
                            {selectedIdentity?.contactEmail && <p className="break-all text-xs text-slate-500 dark:text-zinc-400">{selectedIdentity.contactEmail}</p>}
                        </div>
                    </div>
                </CardContent>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div
                            className={`p-4 rounded-lg border transition-all ${wantAccess
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <Checkbox id="request-access" checked={wantAccess} onCheckedChange={(v) => setWantAccess(!!v)} />
                                <div className="p-2 bg-blue-100 dark:bg-blue-900 rounded-md text-blue-700 dark:text-blue-300">
                                    <Eye className="h-5 w-5" />
                                </div>
                                <label htmlFor="request-access" className="cursor-pointer">
                        ...
                        <div
                            className={`p-4 rounded-lg border transition-all ${wantDeletion
                                ? 'bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800 ring-2 ring-red-500'
                                : 'bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-700 hover:border-slate-300 dark:hover:border-zinc-600'
                                }`}
                        >
                            <div className="flex items-center gap-3">
                                <Checkbox id="request-deletion" checked={wantDeletion} onCheckedChange={(v) => setWantDeletion(!!v)} />
                                <div className="p-2 bg-red-100 dark:bg-red-900 rounded-md text-red-700 dark:text-red-300">
                                    <Trash2 className="h-5 w-5" />
                                </div>
                                <label htmlFor="request-deletion" className="cursor-pointer">
