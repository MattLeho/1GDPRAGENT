...
export async function POST(request:NextRequest){
    ...
    try{
        ...
        if(type!=='text'){
            ...
            const ocr=await executeTask({taskKey:'image.ocr',workflowKey:'vendor.ocr',input:{file_path:intelligencePath,mime_type:match[1]},profileId:authority.profileId});
            if(!ocr.ok)throw new Error(ocr.error.message);text=String((ocr.output as {text?:unknown}).text||'');
        }
        const adjudication=await executeTask({taskKey:'semantic.adjudication',workflowKey:'vendor.ocr',input:{text},configuration:{systemPrompt:'Extract company or vendor names visible in this cookie-consent text. Return only a JSON array of unique names. Do not infer names that are absent.'},profileId:authority.profileId});
