...
function extractRequestAccountDetails(identity: unknown): RequestAccountDetail[] {
    ...
    return requestDetails.flatMap((detail) => {
        ...
        const value = detailRecord.value;
        ...
        return [{ fieldKey: normalizedKey, value: normalizedValue }];
    });
}
...
export async function submitRequest(payload: RequestPayload) {
    ...
    try {
        ...
        const companyUrl = payload.company?.trim();
        const userName = extractIdentityField(payload.identity, ['contactName', 'name', 'fullName', 'full_name', 'displayName']);
        const userEmail = extractIdentityField(payload.identity, ['contactEmail', 'email', 'emailAddress', 'email_address']);
        if (!userName || !userEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(userEmail)) {
            return { success: false, message: 'A valid request identity name and email are required.' };
        }
        ...
        try {
            ...
            return { success: false, message: 'A valid HTTP(S) company or privacy-policy URL is required.' };
        }
        ...
        if (requestDetails.length > 0) {
            try {
                const stored = await Promise.all(requestDetails.map(detail =>
                    requests.addRequestDetail(profileId,newRequestId,detail.fieldKey,encryptCredential(detail.value))
                ));
                if (stored.some(result => !result)) throw new Error('One or more identity fields were not stored');
                ...
                await requests.appendEvent(profileId,{request_id:newRequestId,event_type:'identity_storage_failed',
                    description:'Request drafting stopped because identity details could not be stored',occurred_at:new Date(),
                    actor:'system:request-submission',reason:'Required encrypted identity persistence failed'});
                return {
                    success: false,
                    partial: true,
                    requestId: newRequestId,
                    message: 'A draft record was created, but its identity details could not be stored. Nothing was sent.',
                };
            }
        }

        let policyPersisted = !payload.analysis;
        // Save policy analysis if provided
        if (payload.analysis) {
            try {
                const savedPolicy = await savePolicyAnalysis({
                    requestId: newRequestId,
                    url: companyUrl,
                    dpo_email: payload.analysis.dpo_email || null,
                    company_address: payload.analysis.address || null,
                    data_collected: payload.analysis.data_collected || [],
                    retention_period: payload.analysis.retention_period || null,
                    third_party_sharing: payload.analysis.third_party_sharing || [],
                    summary: payload.analysis.summary || "Analysis from request",
                    risk_score: payload.analysis.risk_score ?? null,
                    provenance: { source: 'request-submission' },
                });
                if (!savedPolicy.success) throw new Error(savedPolicy.error || 'Policy analysis was not persisted');
        ...
        const completionMessage = emailSent
        ...
        return {
            success: true,
            message: policyPersisted
                ? completionMessage
                : `${completionMessage}. The policy analysis could not be stored; review the request before relying on it.`,
            requestId: newRequestId,
            emailSent,
            draftCreated,
            policyPersisted,
