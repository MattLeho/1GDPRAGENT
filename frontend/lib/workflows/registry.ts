...
export const WORKFLOW_DEFINITIONS: readonly WorkflowDefinition[] = [
    ...
    workflow('request.drafting','Request drafting','Draft a reviewed GDPR request.','Requests','frontend:/api/gdpr-agent/draft','draftRequest',['request.drafting']),
    workflow('email.sending','Email sending','Send with the configured server-side email connector.','Email','frontend:built-in-email-transport','sendEmail',[],['email.send']),
    ...
    workflow('graph.query','Typed privacy query','Run one allow-listed read-only privacy tool with assertion and locator citations.','Graph','python:/query','enhancedRag',['graph.explanation']),
    workflow('speech.transcription','Speech transcription','Route local ASR and persist transcript provenance.','Media','python:/execution/invoke','transcription',['speech.transcription']),
...
function normalize(row:Record<string,unknown>):WorkflowPreference {
    return { workflow_key:String(row.workflow_key),execution_mode:row.execution_mode as WorkflowExecutionMode,
...
export async function getWorkflowPreferences():Promise<WorkflowPreference[]> {
    ...
    const stored=new Map(result.rows.map(row=>[row.workflow_key,normalize(row)]));
...
export async function getWorkflowPreference(workflowKey:string):Promise<WorkflowPreference> {
    ...
    return result.rows[0]?normalize(result.rows[0]):defaultPreference(workflowKey);
}
