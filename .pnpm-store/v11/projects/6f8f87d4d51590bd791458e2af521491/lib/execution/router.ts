...
export interface TaskInvocation {
    ...
    configuration?: Record<string, unknown>;
    /** Canonical session-derived profile authority for every persisted execution. */
    profileId: string;
}
...
export async function getTaskRoutes(profileId: string): Promise<TaskRoute[]> {
    const persisted = await pool.query('SELECT * FROM task_routes WHERE profile_id=$1', [profileId]);
...
export async function getTaskRoute(profileId: string, taskKey: string): Promise<TaskRoute> {
    if (!TASKS_BY_KEY.has(taskKey)) throw new Error(`Unknown task: ${taskKey}`);
    const result = await pool.query('SELECT * FROM task_routes WHERE profile_id=$1 AND task_key=$2', [profileId, taskKey]);
...
export function validateTaskRoute(route: TaskRoute): void {
    ...
    if (route.execution_location !== engine.execution_location) throw new Error('Route execution location does not match its engine');
    if (typeof route.enabled !== 'boolean') throw new Error('Route enabled must be boolean');
    if (!Number.isSafeInteger(route.max_concurrency) || route.max_concurrency < 1 || route.max_concurrency > 64) throw new Error('Max concurrency must be an integer from 1 to 64');
    if (!Number.isSafeInteger(route.batch_size) || route.batch_size < 1 || route.batch_size > 10_000) throw new Error('Batch size must be an integer from 1 to 10000');
    if (!Number.isSafeInteger(route.timeout_ms) || route.timeout_ms < 1_000 || route.timeout_ms > 3_600_000) throw new Error('Timeout must be an integer from 1000 to 3600000 milliseconds');
    if (!route.configuration || typeof route.configuration !== 'object' || Array.isArray(route.configuration)) throw new Error('Route configuration must be an object');
    if (!Array.isArray(route.fallback_chain)) throw new Error('Fallback chain must be an array');
    const fallbackIds = new Set(route.fallback_chain.map(fallback => fallback.engine_id));
    if (fallbackIds.size !== route.fallback_chain.length) throw new Error('Fallback engines must be unique');
    if (fallbackIds.has(route.engine_id)) throw new Error('Primary engine cannot also be a fallback');
...
export async function saveTaskRoute(profileId: string, route: TaskRoute): Promise<TaskRoute> {
    validateTaskRoute(route);
    const result = await pool.query(`INSERT INTO task_routes
        (profile_id,task_key,engine_id,provider,model,execution_location,fallback_chain,enabled,max_concurrency,batch_size,timeout_ms,configuration,updated_at)
        VALUES($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9,$10,$11,$12::jsonb,NOW())
        ON CONFLICT(profile_id,task_key) DO UPDATE SET engine_id=EXCLUDED.engine_id,provider=EXCLUDED.provider,
    ...
    [profileId,route.task_key,route.engine_id,route.provider,route.model,route.execution_location,JSON.stringify(route.fallback_chain),route.enabled,route.max_concurrency,route.batch_size,route.timeout_ms,JSON.stringify(route.configuration)]);
    return normalizeRoute(result.rows[0]);
}

export async function getProcessingSettings(profileId: string): Promise<ProcessingSettings> {
    const result = await pool.query('SELECT processing_mode,external_fallback_enabled,approved_external_engines FROM processing_settings WHERE profile_id=$1', [profileId]);
...
export function normalizeProcessingSettings(settings: ProcessingSettings): ProcessingSettings {
    if (!['strict_local','local_first','controlled_cloud'].includes(settings.processing_mode)) throw new Error('Invalid processing mode');
    if (typeof settings.external_fallback_enabled !== 'boolean') throw new Error('Invalid external fallback setting');
    if (!Array.isArray(settings.approved_external_engines) || settings.approved_external_engines.some(engineId => typeof engineId !== 'string')) {
        throw new Error('Invalid approved external engine list');
    }
    const approvedExternalEngines = [...new Set(settings.approved_external_engines)];
    for (const engineId of approvedExternalEngines) {
        const engine = ENGINES_BY_ID.get(engineId);
        if (!engine || engine.execution_location !== 'external') throw new Error(`Approved external engine is invalid: ${engineId}`);
    }
    const normalized: ProcessingSettings = {
        processing_mode: settings.processing_mode,
        external_fallback_enabled: settings.external_fallback_enabled,
        approved_external_engines: approvedExternalEngines,
    };
    return normalized;
}

export async function saveProcessingSettings(profileId: string, settings: ProcessingSettings): Promise<ProcessingSettings> {
    const normalized = normalizeProcessingSettings(settings);
    await pool.query(`INSERT INTO processing_settings(profile_id,processing_mode,external_fallback_enabled,approved_external_engines,updated_at)
        VALUES($1,$2,$3,$4::jsonb,NOW()) ON CONFLICT(profile_id) DO UPDATE SET processing_mode=EXCLUDED.processing_mode,
    ...
    [profileId,normalized.processing_mode,normalized.external_fallback_enabled,JSON.stringify(normalized.approved_external_engines)]);
    return normalized;
}
...
async function ensureAnalysisRun(invocation: TaskInvocation): Promise<string> {
    ...
    const result = await pool.query(`INSERT INTO analysis_runs(profile_id,run_type,status,pipeline_version,configuration,started_at)
        VALUES($1,'task_execution','running','task2-router-v1',$2::jsonb,NOW()) RETURNING id`,
    [invocation.profileId,JSON.stringify({ task_key: invocation.taskKey, workflow_key: invocation.workflowKey || null })]);
...
export async function executeTask(invocation: TaskInvocation): Promise<TaskResult> {
    ...
    const route = await getTaskRoute(invocation.profileId, invocation.taskKey);
    ...
    const privacy = await getProcessingSettings(invocation.profileId);
...
async function startExecutionRecord(invocation: TaskInvocation, runId: string, engine: EngineDefinition, model: string | null): Promise<string> {
    const result = await pool.query(`INSERT INTO execution_records
        (profile_id,analysis_run_id,task_key,workflow_key,engine_id,provider,model,execution_location,source_artifact_ids,status,input_size)
        VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9::uuid[],'running',$10) RETURNING id`,
    [invocation.profileId,runId,invocation.taskKey,invocation.workflowKey || null,engine.engine_id,engine.provider,model,engine.execution_location,invocation.sourceArtifactIds || [],byteSize(invocation.input)]);
    return result.rows[0].id;
}
