...
async function proxy(request:NextRequest,context:{params:Promise<{path?:string[]}>}){
  ...
  try{
    ...
    const response=await fetch(url,{method:request.method,body:body||undefined,headers:intelligenceAuthorityHeaders(authority.profileId),cache:'no-store',signal:AbortSignal.timeout(120_000)});
