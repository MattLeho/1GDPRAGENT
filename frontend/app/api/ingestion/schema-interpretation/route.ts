    ...
    try {
        ...
        const result = await pool.query(
            `SELECT sir.id,sir.analysis_run_id,sir.execution_record_id,sir.sample_manifest,sf.fingerprint_hash
             FROM schema_interpretation_requests sir JOIN structure_fingerprints sf ON sf.id=sir.structure_fingerprint_id
             WHERE sir.id=$1`, [body.interpretation_request_id],
        );
        ...
        const routed = await executeTask3Bundle(manifest);
