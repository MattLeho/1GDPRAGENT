 ...
 */
export async function getWebhookUrl(webhookId: string, profileId: string): Promise<string | null> {
    try {
        const result = await pool.query(
            'SELECT webhook_url FROM n8n_webhooks WHERE webhook_name = $1 AND profile_id = $2 AND is_active = true',
            [webhookId, profileId]
        );
 ...
 */
export async function getAllWebhookUrls(profileId: string): Promise<Record<string, string | null>> {
    const urls: Record<string, string | null> = {};

    for (const { id } of webhookMappings) {
        urls[id] = await getWebhookUrl(id, profileId);
    }
 ...
 */
export async function isWebhookConfigured(webhookId: string, profileId: string): Promise<boolean> {
    const url = await getWebhookUrl(webhookId, profileId);
