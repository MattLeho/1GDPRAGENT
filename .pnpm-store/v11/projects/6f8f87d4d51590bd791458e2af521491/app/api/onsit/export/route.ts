...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        let whereClause = "WHERE n.profile_id = $profileId AND (n.source = 'onsit' OR n.object_source = 'onsit')";
        const params: Record<string, unknown> = { profileId: authority.profileId };
        ...
        return NextResponse.json({
            error: 'ONSIT graph export is temporarily unavailable',
        }, { status: 503 });
    } finally {
        await session.close();
    }
