...
export async function callN8NWebhook<T = unknown>(
    ...
    const configuredUrl = await getWebhookUrl(webhookType, profileId);
 ...
 */
export async function analyzePolicy(url: string, profileId: string): Promise<N8NResponse<PolicyAnalysisResult>> {
    return callN8NWebhook<PolicyAnalysisResult>('analyzePolicy', {
        url,
        timestamp: new Date().toISOString(),
    }, profileId, { timeout: 60000 }); // Policy analysis can take longer
}
...
}, profileId: string): Promise<N8NResponse<{ subject: string; body: string; to: string }>> {
    return callN8NWebhook('draftRequest', params, profileId);
}
...
export async function sendEmail(params: {
    ...
    body: string;
    replyTo?: string;
}, profileId: string): Promise<N8NResponse<{ messageId: string }>> {
    return callN8NWebhook('sendEmail', params, profileId);
}
...
export async function testImapConnection(params: {
    ...
    port: number;
    password: string;
}, profileId: string): Promise<N8NResponse<{ connected: boolean; mailboxCount?: number }>> {
    return callN8NWebhook('testImap', params, profileId, { timeout: 15000 });
}
...
export async function ingestToKnowledgeGraph(params: {
    ...
    categories: string[];
}, profileId: string): Promise<N8NResponse<{ nodesCreated: number; relationshipsCreated: number }>> {
    return callN8NWebhook('ingestData', params, profileId);
}
...
export async function ingestIdentity(params: {
    ...
    usernames?: string[];
    notes?: string;
}, profileId: string): Promise<N8NResponse<{ personaId: string; entitiesCreated: number }>> {
    return callN8NWebhook('ingestIdentity', params, profileId);
}
