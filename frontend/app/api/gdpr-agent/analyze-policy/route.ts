...
export async function POST(request:NextRequest){
    try{
        ...
        const sourceResponse=await fetch(`${baseUrl}/extract/policy-claims`,{method:'POST',headers:intelligenceAuthorityHeaders(authority.profileId),body:JSON.stringify({content:policyText,policy_key:new URL(url).hostname,version_label:acquiredAt.toISOString(),retrieved_at:acquiredAt.toISOString(),authorisation_basis:'User-initiated acquisition of a publicly presented privacy policy',source_uri:url,controller_key:company||new URL(url).hostname,claims}),signal:AbortSignal.timeout(120_000)});
