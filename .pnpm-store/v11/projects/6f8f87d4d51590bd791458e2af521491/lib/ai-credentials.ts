...
import { decryptCredential as decryptCanonicalCredential, isCanonicalCiphertext } from '@/lib/secure-credentials';
...
function decrypt(encryptedText: string): string | null {
    try {
        if (isCanonicalCiphertext(encryptedText)) {
            return decryptCanonicalCredential(encryptedText);
        }
        // Read-only compatibility for local credentials saved before the
        // authenticated AES-GCM format was introduced. All new writes use GCM.
