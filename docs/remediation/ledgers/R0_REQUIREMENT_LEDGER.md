...
This is the lead-owned classification ledger for all original Task 1--6 requirement units (including Task 3A), reconciled against the live post-R2 checkout on 2026-08-14. Evidence classes are independent: C=code, M=migration, T=automated test, R=authenticated runtime. The original 2026-07 audit observations remain frozen in [the documentation-audit evidence](../evidence/subagent-plan-doc-audit.md); the rows below are the current R0 classification after accepted R1/R2 repairs and the current R0 authenticated baseline.
...
| T1 | §5 stable graph IDs | C/M/T; R authenticated R0 graph double | IMPLEMENTED_NOT_INTEGRATED | populated non-test graph runtime (R6) |
...
| T1 | §10 preserve useful functions | C/T; R authenticated subset | PARTIAL | live provider/connector/graph integrations (R3/R4/R6) |
...
| T2 | §2 Engine Registry/no Google fallback | C/M/T; R hermetic no-provider chat | PARTIAL | MODEL-008 and live engine routing (R3) |
...
| T2 | §9 settings IA | C/T; R authenticated profile/connector subset | PARTIAL | live connectors and remaining responsive IA (R4/R5) |
...
| T3 | Wave 4 router/providers/benchmark | C/M/T; R hermetic no-provider chat | PARTIAL | MODEL-008 and configured provider benchmark (R3) |
...
| T5 | Wave 1 registry/runtime/scheduler | C/M/T; R authenticated R0 connector double | IMPLEMENTED_NOT_INTEGRATED | live connector runtime/scheduler (R4) |
...
| T5 | Wave 7 settings controls | C/T; R authenticated selector/action | PARTIAL | live permissions/retention/deletion controls (R4/R5) |
...
| T6 | Wave 4 hypotheses/DSAR | C/M/T; R request baseline green | IMPLEMENTED_NOT_INTEGRATED | populated hypothesis/DSAR runtime (R6) |
...
| T6 | Wave 7 graph UI/API | C/M/T; R authenticated R0 graph double | IMPLEMENTED_NOT_INTEGRATED | populated non-test graph runtime (R6) |
...
| T3-4A provider/runtime adapters | C/M/T/R-hermetic no-provider | PARTIAL | MODEL-008/live providers (R3) |
...
| T5-1A registry/sync runtime | C/M/T/R-test double | IMPLEMENTED_NOT_INTEGRATED | live connector runtime (R4) |
...
| T5-6C controller-erasure integration | C/M/T/R-none | TEST_ONLY | isolated destructive acceptance (R4/R7) |
| T5-7A connector settings | C/T/R-authenticated selector | PARTIAL | live connector configuration (R4) |
...
| T6-4A hypothesis detectors | C/M/T/R-none | IMPLEMENTED_NOT_INTEGRATED | populated runtime (R6) |
| T6-4B DSAR templates | C/T/R-none | IMPLEMENTED_NOT_INTEGRATED | populated runtime (R6) |
| T6-4C resolution service | C/M/T/R-none | IMPLEMENTED_NOT_INTEGRATED | populated runtime (R6) |
...
| T6-7A temporal/compare graph API | C/M/T/R-authenticated test double | IMPLEMENTED_NOT_INTEGRATED | populated graph (R6) |
| T6-7B profile-layer graph API | C/M/T/R-authenticated test double | IMPLEMENTED_NOT_INTEGRATED | populated graph (R6) |
| T6-7C modes/navigation | C/T/R-authenticated shell | PARTIAL | populated graph interactions (R6) |
