...
const stepDescriptions: Record<string, string> = {
    ...
    username: 'Searching enabled username providers',
    ...
    compile: 'Preparing findings for review',
};
...
function statusLabel(status: DiscoveryProgressResponse['status']): string {
    if (status === 'completed') return 'Discovery complete';
    if (status === 'failed') return 'Discovery failed';
    if (status === 'queued') return 'Waiting to start';
    return 'Searching enabled providers';
}
...
export async function GET(
    ...
    try {
        ...
        const progressResponse: DiscoveryProgressResponse = {
            taskId: data.scan_id,
            status: mappedStatus,
            progress: progressPercent(data.progress),
            currentStep: data.current_step || statusLabel(mappedStatus),
