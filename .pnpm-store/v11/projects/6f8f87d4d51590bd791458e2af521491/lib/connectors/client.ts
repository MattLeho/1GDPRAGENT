import type {ConnectorHealth,ConnectorInstance,SourceConnectorDefinition} from './types';
...
export interface SyncQueuedResult{queued:boolean;task_id:string}
...
export const syncConnector=(id:string,backfill=false)=>request<SyncQueuedResult>(`/instances/${encodeURIComponent(id)}/sync?backfill=${backfill}`,{method:'POST'});
export const fetchConnectorHealth=(id:string)=>request<ConnectorHealth>(`/instances/${encodeURIComponent(id)}/health`);
