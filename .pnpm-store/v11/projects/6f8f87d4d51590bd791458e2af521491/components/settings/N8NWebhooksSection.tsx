        ...
        async function loadWebhooks() {
            try {
                ...
                if (res.ok) {
                    ...
                    toast.error('Could not load N8N webhook settings');
                }
            } catch (e) {
    ...
    const handleSave = async (data: N8NWebhooksForm) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast.error(error.error || 'Failed to save webhooks');
            }
        } catch (error) {
                ...
                <CardContent className="space-y-3">
                    {webhookConfigs.map(webhook => {
                        ...
                        return (
                            <div
                                key={webhook.id}
                                className="flex flex-col gap-3 rounded-lg border bg-zinc-50/50 p-3 dark:bg-zinc-900/50 sm:flex-row sm:items-center"
                            >
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="relative flex-1">
                                            <Input
                                                ...
                                                className="h-11 pr-12 text-sm font-mono"
                                            />
                                            <Button
                                                type="button"
                                                variant="ghost"
                                                ...
                                                aria-label={isShown ? `Hide ${webhook.name} URL` : `Show ${webhook.name} URL`}
                                            >
                                                {isShown ? (
                    ...
                    {/* N8N Link */}
                    <div className="flex flex-col gap-3 rounded-lg border border-purple-200 bg-purple-50 p-3 dark:border-purple-800 dark:bg-purple-900/20 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-2">
