...
export async function POST(request: Request) {
  ...
  const results = await Promise.all(nodes.map(async (node: unknown) => {
    const response = await fetch(`${intelligenceUrl}/evidence/manual-node`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(node), cache: 'no-store' })
