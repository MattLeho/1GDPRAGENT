...
export async function GET(request: NextRequest) {
    ...
    return NextResponse.json({tasks:TASK_DEFINITIONS,engines:ENGINE_DEFINITIONS,routes:await getTaskRoutes(authority.profileId)});
}

export async function POST(request: NextRequest) {
    const authority = await requireApiSession(request);
    if (authority instanceof NextResponse) return authority;
    try { return NextResponse.json({success:true,route:await saveTaskRoute(authority.profileId, await request.json() as TaskRoute)}); }
