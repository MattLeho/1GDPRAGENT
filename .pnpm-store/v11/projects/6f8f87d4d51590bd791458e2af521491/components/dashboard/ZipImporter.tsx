...
import {
    Upload, FileArchive, File, FileText, Image as ImageIcon, FileSpreadsheet,
    FileAudio, FileVideo, Loader2, X, Sparkles, CheckCircle2,
    AlertCircle, Clock, ArrowRight, Database, Brain, Eye,
    FileCode, LayoutList, Pencil, Trash2, Check
} from 'lucide-react';
...
const PROCESSING_STAGES = [
    ...
    { id: 'record', label: 'Recording', description: 'Saving evidence for review', icon: Database },
...
export function ZipImporter({ requestId, onComplete }: ZipImporterProps) {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const updatedFiles: ProcessedFile[] = files.map(f => {
                const serverFile = uploadData.files.find((sf: { id: string; fileName: string }) =>
            ...
            let processedFiles = [...updatedFiles];

            for (const file of updatedFiles) {
                if (!file.id) {
                    processedFiles = processedFiles.map(candidate => candidate === file ? {
                        ...candidate,
                        status: 'error',
                        errorMessage: 'The server did not return a persisted file ID',
                    } : candidate);
                    setFiles(processedFiles);
                    addLog(`✗ Error: ${file.name} was not persisted`, 'error');
                    processedCount++;
                ...
                try {
                    ...
                    const completedFile: ProcessedFile = {
                        ...file,
                        status: 'completed',
                        processingStage: 'evidence_recorded',
                        processingProgress: 100,
                        analysisRunId: result.analysis_run_id,
                        artifactId: result.artifact_id,
                        ingestionStatus: result.ingestion_status,
                        warnings: Array.isArray(result.warnings) ? result.warnings : [],
                    };
                    processedFiles = processedFiles.map(candidate =>
                        candidate.id === file.id ? completedFile : candidate
                    );
                    setFiles(processedFiles);
                    addLog(`✓ Evidence recorded: ${file.name}`, 'success', file.id);
                    for (const warning of completedFile.warnings || []) {
                        addLog(`⚠ ${file.name}: ${warning}`, 'progress', file.id);
                    }
                } catch (err) {
            ...
            // Step 3: Make the evidence boundary explicit. The processing API records
            // canonical evidence; graph projection happens only after human review.
            setCurrentStage('record');
            setOverallProgress(95);
            const completedFiles = processedFiles.filter(file => file.status === 'completed' && file.id);
            addLog(`Evidence recorded for ${completedFiles.length} files; awaiting review before graph projection`, 'success');
            ...
            toast.success('Files processed successfully', {
                description: `${completedFiles.length} of ${processedCount} files recorded as evidence for review`
            });

            onComplete?.(processedFiles.filter(file => file.status === 'completed'));
        } catch (error) {
