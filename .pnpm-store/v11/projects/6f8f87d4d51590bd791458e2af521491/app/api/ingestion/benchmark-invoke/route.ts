...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const route = await getTaskRoute(authority.profileId, body.bundle.task_key);
        ...
        const audit = executionRecordId
            ? (await pool.query('SELECT engine_id,provider,model,execution_location FROM execution_records WHERE id=$1 AND profile_id=$2', [executionRecordId, authority.profileId])).rows[0]
            : null;
